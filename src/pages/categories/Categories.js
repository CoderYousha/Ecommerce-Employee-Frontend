...
function Categories() {
    ...
    const getCategories = async () => {
        ...
        if (result.status === 200) {
            ...
            setCategoriesCounts(result.data.data.pagination.total);
                                                    ...
                                                    <TableBody>
                                                        {categories.map((category, index) => (
                                                            <StyledTableRow key={index} className="hover:bg-gray-200 duration-100 cursor-pointer">
                                                                <StyledTableCell align={language === 'en' ? "left" : "right"} className="">{language === 'en' ? category.name_en : category.name_ar}</StyledTableCell>
